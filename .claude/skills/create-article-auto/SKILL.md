# Skill : Creer un article evergreen SEO (full auto) — Le Mag Habitat
...
- `data/authors.yaml` present (systeme d'auteurs partage, 3 profils sur ce blog).
- `content/blog/` et `content/en/blog/` existent. **Sur ce blog, le FR vit a la racine de `content/`, pas sous `content/fr/`** : le `hugo.toml` monte `content` (en excluant `en/**`) sur la langue `fr`, et `content/en` sur la langue `en` (voir les blocs `[[module.mounts]]`). Un fichier ecrit ailleurs n'appartient a aucune langue et Hugo l'ignore silencieusement. Ne pas copier le chemin `content/fr/blog/` d'un autre blog du reseau, ca n'existe pas ici.
- Remote git `origin` configure, acces push (repo `analytics-ds/le-mag-habitat`).
- Cle `DATAFER_API_KEY` et `CRAZYSERP_API_KEY` exportees par le prompt de la routine (source SERP nominale). Outil `WebSearch` disponible en repli. Si les deux manquent, la skill degrade en mode "kw seul" sans echouer.

## Regle de debrandage (obligatoire, propre a ce blog)

Le Mag Habitat est un blog **Type A**, exclusivement reserve au client Murfy, mais **non brande** : le site se presente comme un magazine maison independant, sans logo ni mention Murfy visible. Consequence directe pour la redaction automatique :

- **Ne jamais mentionner "Murfy", "datashake" ni aucun nom de marque du client** dans le corps, le title, la meta description, les alt text ou la FAQ d'un article.
- Aucun lien sortant obligatoire vers un site client sur ce blog (contrairement aux blogs qui citent explicitement leurs clients).
- Le ton reste celui d'un magazine maison generique, jamais publicitaire pour une marque en particulier.
...
**NE JAMAIS ecrire le nom du site dans le `title` du frontmatter.** Le theme l'ajoute deja tout seul : `themes/le-mag-habitat-theme/layouts/_default/baseof.html` rend `{{ if .IsHome }}{{ .Site.Title }}{{ else }}{{ .Title }} | {{ .Site.Title }}{{ end }}`. Mettre le suffixe dans le frontmatter le fait sortir **deux fois** dans la balise. Le H1 (`layouts/_default/single.html`) est rendu directement sur `{{ .Title }}`, donc pas de risque de doublon cote H1 tant que le frontmatter reste propre.

- Format du frontmatter : `[Kw] : [angle]`, **sans nom de site, sans separateur final**
- Contient le `kw` en debut de title
- **L'angle est TOUJOURS invente par l'agent a partir du `kw` et de l'analyse SERP, jamais recopie d'une source externe** (aucun champ d'angle n'existe dans `roadmap.yaml` sur ce blog, c'est volontaire).
- **Le budget de 60 caracteres porte sur le title RENDU**, donc frontmatter + suffixe `" | Le Mag Habitat"` (17 caracteres). Le maximum du frontmatter est donc **43 caracteres**.
- Controle avant de continuer : `python3 -c "print(len('<title frontmatter>') + 17)"` doit donner 60 au maximum. Si ca depasse, raccourcir l'angle, jamais le `kw`.
- **Une seule option, choix direct** (pas de 3 options comme en interactif)
- Meme regle pour la traduction EN (suffixe `" | Le Mag Habitat"`, meme longueur).
...
# construire {"editorHtml": "..."} depuis content/blog/<slug>.md
PY
...
Le sandbox cloud n'a PAS Hugo pre-installe, et `apt` fournit une version trop ancienne (0.123.x) qui fait echouer le build de certains sites du reseau. **Ce blog est pinne sur Hugo extended v0.139.0** (voir `.github/workflows/*.yml`, champ `hugo_extended_..._linux-amd64.deb`), une version differente du defaut 0.161.1 utilise sur le reste du parc. Installer EXACTEMENT cette version :

```bash
wget -q -O /tmp/hugo.deb https://github.com/gohugoio/hugo/releases/download/v0.139.0/hugo_extended_0.139.0_linux-amd64.deb \
  && (sudo dpkg -i /tmp/hugo.deb 2>/dev/null || { dpkg-deb -x /tmp/hugo.deb /tmp/hugobin && export PATH="/tmp/hugobin/usr/local/bin:$PATH"; })
hugo version   # doit afficher v0.139.0 extended
```
...
```bash
mkdir -p /tmp/hugobin-npm && cd /tmp/hugobin-npm && npm init -y >/dev/null 2>&1 && npm install hugo-extended@0.139.0
