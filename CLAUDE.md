...
- **Nom du site** : Le Mag Habitat
- **Description (FR)** : Le magazine qui accompagne vos travaux, votre énergie et l'entretien de votre maison au quotidien.
- **Description (EN)** : The magazine that guides your home renovation, energy and everyday maintenance projects.
- **URL** : https://le-mag-habitat.fr/ (domaine pas encore connecte, a faire via `/github-setup`)
- **Client lie** : Murfy (PBN type A, non brande, exclusivement reserve a Murfy — voir `Clients/Murfy/CLIENT.md`)
- **Couleurs** : fond blanc casse chaud `#FAF6F1`, fond alt `#F0E6DA`, accent terracotta `#C1633B` (primary/cta), accent vert sauge `#8A9A7E`, texte anthracite chaud `#3A322C`, bordures `#E4D9CB`
- **Polices** : titres Fraunces (serif chaleureuse), corps Karla (sans-serif), UI Inter (sans-serif)
- **Langue principale** : francais (la version EN en sous-dossier `/en/` est TOUJOURS active)
- **Categories (FR ↔ EN)** :
  - Énergie et consommation / Energy and consumption
  - Équipement et réparation / Appliances and repair
  - Travaux et rénovation / Renovation and building work
  - Aménagement et décoration / Interior design and decor
  - Entretien de la maison / Home maintenance
- **Auteur principal du site** : `claire-beaumont`
- **A faire** : generer `static/images/og-default.jpg` (1200x630px), generer/associer les avatars auteurs manquants, connecter le domaine le-mag-habitat.fr via `/github-setup`
...
- **Toujours mettre les accents francais (e, a, e, u, c, etc.) dans tout contenu produit** (titres, textes, meta descriptions, alt text, noms de categories affiches...), quel que soit le type de creation (site, article, page statique, donnees). Seuls les slugs d'URL et noms de fichiers/dossiers restent sans accent
...
- Chaque article doit avoir un champ `faq` dans le frontmatter (liste de questions/reponses) pour generer automatiquement le schema FAQPage JSON-LD. Minimum 3 questions. Ce champ alimente aussi un bloc FAQ visible en accordeon en bas de chaque article (genere automatiquement par `single.html` depuis le frontmatter, aucune action manuelle supplementaire requise)
